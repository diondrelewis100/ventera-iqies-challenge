...
module.exports =function (source, target, input){
    switch(source){
           ...
           case units["CELSIUS"]:
               //converting all cases where input unit is "CELSIUS"
...
const convertValFromFahrenheit = (target, input) =>{
    switch(target){
        ...
        case units["CELSIUS"]:
             ...
             return ((parseFloat(input) - parseFloat(measurement[`FahrenheitTo${target.replace(/ /g, '_')}`])) / 1.8);
        case units["KELVIN"]:
             //Formula to convert from "FAHRENHEIT" to "KELVIN"
             return ((parseFloat(input) + parseFloat(measurement[`FahrenheitTo${target.replace(/ /g, '_')}`])) * (5.0/9.0));
        case units["RANKINE"]:
             //Formula to convert from "FAHRENHEIT" to "RANKINE"
             return parseFloat(input) + parseFloat(measurement[`FahrenheitTo${target.replace(/ /g, '_')}`]);
   }
}

//calculating value in target unit where input source unit is "Celsius"
    ...
    switch(target){
        ...
        case units["FAHRENHEIT"]:
            //Formula to convert from "CELSIUS" to "FAHRENHEIT"
             return ((parseFloat(input) * (1.8)) + parseFloat(measurement[`CelciusTo${target.replace(/ /g, '_')}`]));
        case units["KELVIN"]:
            //Formula to convert from "CELSIUS" to "FAHRENHEIT"
             return (parseFloat(input) + parseFloat(measurement[`CelciusTo${target.replace(/ /g, '_')}`]));
        case units["RANKINE"]:
            //Formula to convert from "CELSIUS" to "FAHRENHEIT"
            return ((parseFloat(input) + parseFloat(measurement[`CelciusTo${target.replace(/ /g, '_')}`])) * (1.8));
   } 
}
    ...
    switch(target){
        ...
        case units["CELSIUS"]:
            //Formula to convert from "KELVIN" to "CELSIUS"
             return (parseFloat(input) - parseFloat(measurement[`KelvinTo${target.replace(/ /g, '_')}`]));
        case units["FAHRENHEIT"]:
            //Formula to convert from "KELVIN" to "FAHRENHEIT"
            return ((parseFloat(input) * (1.8)) - parseFloat(measurement[`KelvinTo${target.replace(/ /g, '_')}`]));
        case units["RANKINE"]:
...
const convertValFromRankine = (target, input) =>{
    switch(target){
        case units["RANKINE"]:
             return parseFloat(input);
        case units["CELSIUS"]:
            //Formula to convert from "RANKINE" to "CELSIUS"
             return ((parseFloat(input) - parseFloat(measurement[`RankineTo${target.replace(/ /g, '_')}`])) * (5.0/9.0));
        case units["KELVIN"]:
            //Formula to convert from "RANKINE" to "KELVIN"
             return (parseFloat(input) * (5.0/9.0));
        case units["FAHRENHEIT"]:
            //Formula to convert from "RANKINE" to "FAHRENHEIT"
             return parseFloat(input) - parseFloat(measurement[`RankineTo${target.replace(/ /g, '_')}`]);
   }
}
...
const convertValFromTableSpoon = (target, input) =>{
    if(units["TABLE_SPOON"] == target){
        ...
        return (parseFloat(input) * parseFloat(measurement[`TablespoonTo${target.replace(/ /g, '_')}`]));
    }
}
...
const convertValFromCubicInch = (target, input) =>{
    if(units["CUBIC_INCH"] == target){
        ...
        return (parseFloat(input) * parseFloat(measurement[`CubicinchTo${target.replace(/ /g, '_')}`]));
    }
}
...
const convertValFromCubicFeet = (target, input) =>{
    if(units["CUBIC_FEET"] == target){
        ...
        return (parseFloat(input) * parseFloat(measurement[`CubicfeetTo${target.replace(/ /g, '_')}`]));
    }
}
...
const convertValFromCup = (target, input) =>{
    if(units["CUP"] == target){
        ...
        return (parseFloat(input) * parseFloat(measurement[`CupTo${target.replace(/ /g, '_')}`]));
    }
}
...
const convertValFromLiter = (target, input) =>{
    if(units["LITER"] == target){
        ...
        return (parseFloat(input) * parseFloat(measurement[`LiterTo${target.replace(/ /g, '_')}`]));
    }
}
...
const convertValFromGallon = (target, input) =>{
    if(units["GALLON"] == target){
        ...
        return (parseFloat(input) * parseFloat(measurement[`GallonTo${target.replace(/ /g, '_')}`]));
    }
}
